...
## Use `setwd()` if needed
#setwd("/home/jdoe/Workspaces/dsc520") - NOT NEEDED

## Load the file `data/tidynomicon/person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv('data/tidynomicon/person.csv', stringsAsFactors = TRUE) # set TRUE to match assignment output
str(person_df1)
...
scores_df <- read.csv('data/scores.csv', stringsAsFactors = TRUE) # set TRUE to match assignment output
summary(scores_df)
